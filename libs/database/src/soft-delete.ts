...
import { withAdmin } from './tenant-db';
...
export async function softDelete(db: DrizzleDB, table: PgTable, id: string): Promise<void> {
...
export async function restoreDeleted(db: DrizzleDB, table: PgTable, id: string): Promise<void> {
  assertSoftDeletable(table);
  const t = table as TableRef;
