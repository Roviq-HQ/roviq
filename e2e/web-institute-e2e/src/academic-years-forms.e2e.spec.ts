...
test.describe('Create academic year dialog', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator('[data-test-id="academic-years-create-dialog"]')).toBeVisible();
  });

  test('dialog opens with the expected fields and labels (CLFYD/DNMPQ)', async ({ page }) => {
    const dialog = page.locator('[data-test-id="academic-years-create-dialog"]');
    await expect(
      dialog.locator('[data-test-id="academic-years-create-label-input"]'),
    ).toBeVisible();
    await expect(dialog.locator('[data-test-id="academic-years-create-start-date"]')).toBeVisible();
    await expect(dialog.locator('[data-test-id="academic-years-create-end-date"]')).toBeVisible();

    // FVOLK — explicit submit text
    await expect(dialog.locator('[data-test-id="academic-years-create-submit-btn"]')).toBeVisible();
  });

  test('blank submit shows three translated field errors', async ({ page }) => {
    const dialog = page.locator('[data-test-id="academic-years-create-dialog"]');
    await dialog.locator('[data-test-id="academic-years-create-submit-btn"]').click();
  ...
  test('label input uses the Indian academic-year placeholder format', async ({ page }) => {
    ...
    const labelInput = page.locator('[data-test-id="academic-years-create-label-input"]');
...
test.describe('Edit academic year sheet', () => {
  ...
  test('Edit button opens the sheet with expected fields (CLFYD/FVOLK)', async ({ page }) => {
    ...
    const editButton = page.locator('[data-test-id="academic-years-edit-btn"]').first();
    await expect(editButton).toBeVisible({ timeout: 10_000 });

    await editButton.click();

    const sheet = page.locator('[data-test-id="academic-years-edit-sheet"]');
    await expect(sheet).toBeVisible();

    // FVOLK — save button present
    await expect(sheet.locator('[data-test-id="academic-years-edit-save-btn"]')).toBeVisible();
  });
});
...
test.describe('Institute settings · address form', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator('[data-test-id="settings-address-postal-code"]')).toBeVisible({
      timeout: 15_000,
    });
  });

  test('address section renders with PIN code and state combobox', async ({ page }) => {
    const pin = page.locator('[data-test-id="settings-address-postal-code"]');
    await expect(pin).toHaveAttribute('maxlength', '6');

    // GGPVY — state is a combobox (cmdk)
    await expect(page.locator('[data-test-id="settings-address-state"]')).toBeVisible();
  });
  ...
  test('PIN 122001 auto-fills city/district (HBCFO)', async ({ page }) => {
    ...
    await expect(page.locator('[data-test-id="settings-address-city"]')).not.toHaveValue('', {
      timeout: 8_000,
    });
    await expect(page.locator('[data-test-id="settings-address-district"]')).not.toHaveValue('', {
      timeout: 8_000,
    });
  });
});
