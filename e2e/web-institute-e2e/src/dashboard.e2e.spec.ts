...
test.describe('Dashboard', () => {
  ...
  test('Quick Links section shows 4 links', async ({ page }) => {
    await expect(page.locator('[data-test-id="dashboard-quick-links"]')).toBeVisible();

    await expect(page.locator('[data-test-id="dashboard-quick-link-standards"]')).toBeVisible();
    await expect(page.locator('[data-test-id="dashboard-quick-link-subjects"]')).toBeVisible();
    await expect(page.locator('[data-test-id="dashboard-quick-link-users"]')).toBeVisible();
    await expect(page.locator('[data-test-id="dashboard-quick-link-settings"]')).toBeVisible();
  });

  test('quick links navigate to correct pages', async ({ page }) => {
    await page.locator('[data-test-id="dashboard-quick-link-standards"]').click();
    ...
    await page.locator('[data-test-id="dashboard-quick-link-settings"]').click();
